...
def changepassword(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Ошибка, изменение пароля', extra_tags='alert alert-warning alert-dismissible show')
            return redirect('accounts:changepassword')
...
def register_user_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Аккаунт создан для {0} '.format(username),
                             extra_tags='alert alert-success alert-dismissible show')
            return redirect('accounts:register')
        else:
            messages.error(request, 'Имя пользователя или пароль недействительны',
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user and user.is_active:
                ...
                messages.error(request, 'Аккаунт недействителен', extra_tags='alert alert-error alert-dismissible show')
                return redirect('accounts:login')

        else:
            return HttpResponse('Данные не действительны')
...
def user_profile_view(request):
    ...
    return HttpResponse("Нет прав для доступа")
